...
def scrape_shl_catalog():
    ...
    BASE_CATALOG_URL = "https://www.shl.com/solutions/products/product-catalog/?start={}"
    assessments = []

    headers = {"User-Agent": "Mozilla/5.0"}
    for type_id in range(0, 12):  # Loop from type=1 to type=12
        count = 12*type_id
        ...
        try:
            ...
            product_rows = soup.select('tr[data-course-id]')
            if not product_rows:
...
def main():
    ...
    if assessments:
        ...
        logger.info("Saved data to shl_assessments.csv")
